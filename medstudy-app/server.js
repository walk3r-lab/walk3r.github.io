...
function importSubjectName(root,path){let r=String(root||'').toUpperCase();if(r==='BIOCHEMISTRY')return 'Biochemistry';if(r==='BEHAVIOURAL SCIENCE'||r==='BEHAVIORAL SCIENCE')return 'Behavioral Science';if(r==='DR BEDA SLIDES')return 'Anatomy';if(r==='MODULES')return /SOCIAL AND BEHAVIOURAL|SOCIAL AND BEHAVIORAL|BEHAVIOUR|BEHAVIOR|PSYCH|HEALTHCARE|ANTHROPOLOGY|SOCIOLOGY/i.test(path)?'Behavioral Science':'Behavioral Science';return null}
...
LECTURE TRANSCRIPT:
${source.slice(0,220000)}`;let raw=await ai(prompt),pack=JSON.parse(cleanJson(raw));return pack}
